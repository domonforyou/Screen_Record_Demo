...
#endif
    //domon: stop the encode to move other place
    //StartThread();
}
...
bool VideoEncoder::EncodeFrame(AVFrame* frame) {
	...
	if(frame != NULL) {
        ...
        assert(frame->width == GetCodecContext()->width);
        assert(frame->height == GetCodecContext()->height);
#endif
#if SSR_USE_AVFRAME_FORMAT
        assert(frame->format == GetCodecContext()->pix_fmt);
#endif
#if SSR_USE_AVFRAME_SAR
        assert(frame->sample_aspect_ratio.num == GetCodecContext()->sample_aspect_ratio.num);
        assert(frame->sample_aspect_ratio.den == GetCodecContext()->sample_aspect_ratio.den);
#endif
	}

#if SSR_USE_AVCODEC_SEND_RECEIVE
	// send a frame
    AVFrame *avframe = (frame == NULL)? NULL : frame;
	try {
    ...
    //domon: free avframe data and itself
	...
	for( ; ; ) {
        ...
        packet->size = 0;
#endif
        int res = avcodec_receive_packet(GetCodecContext(), packet);
        if(res == 0) { // we have a packet, send the packet to the muxer
            GetMuxer()->AddPacket(GetStream()->index, packet);
	...
	// encode the frame
	int got_packet;
    if(avcodec_encode_video2(GetCodecContext(), packet, (frame == NULL)? NULL : frame, &got_packet) < 0) {
