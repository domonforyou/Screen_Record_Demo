...
static void Free_avpacket(AVPacket *packet){
...
void Muxer::Start() {
    ...
    char sdp[2048];
    av_sdp_create(&m_format_context,1, sdp, sizeof(sdp));
    printf("%s\n",sdp);
    fflush(stdout);
...
void Muxer::AddPacket(unsigned int stream_index, AVPacket* packet) {
	assert(m_started);
	assert(stream_index < m_format_context->nb_streams);
	StreamLock lock(&m_stream_data[stream_index]);
    lock->m_packet_queue.push_back(packet);
}
        ...
        while(!m_should_stop) {
			...
			for(unsigned int i = 0; i < m_format_context->nb_streams; ++i) {
				...
				if(lock->m_packet_queue.empty()) {
					...
					current_stream = i;
                    packet = lock->m_packet_queue.front();
            ...
            if(packet->dts != (int64_t) AV_NOPTS_VALUE)
                packet_time = (double) packet->dts * ToDouble(codec_context->time_base);
            else if(packet->pts != (int64_t) AV_NOPTS_VALUE)
                packet_time = (double) packet->pts * ToDouble(codec_context->time_base);
			if(packet_time > total_time)
				total_time = packet_time;

			// prepare packet
            packet->stream_index = current_stream;
